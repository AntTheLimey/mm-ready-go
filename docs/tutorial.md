...
This tutorial walks through all three mm-ready-go modes using a local
PostgreSQL database with a deliberately problematic schema. You will see
exactly what mm-ready-go finds and how to read the reports.
...
- mm-ready-go installed (`go install github.com/pgEdge/mm-ready-go@latest`)
  or built from source (`make build`)
...
Scan mode checks a vanilla PostgreSQL database for Spock compatibility
issues. This is the primary use case -- run it before installing Spock.
...
- **Summary bar** — severity counts (Critical / Warning / Consider / Info)
- **Readiness verdict** — READY, CONDITIONALLY READY, or NOT READY
- **Findings by severity** — Critical first, then Warning, Consider, Info
- **To-Do list** — actionable remediation steps at the bottom

Every finding includes what was found, why it matters for replication, and
how to fix it. The tool queries `pg_catalog` directly -- it is fast and sees
...
get a `pg_dump --schema-only` SQL file. Analyze mode handles that.
...
Analyze mode runs 19 of the 57 checks -- those that can work from schema
...
The structural findings (primary keys, constraints, foreign keys) are
identical to what a live scan produces.
...
In production you would run this for a full business cycle -- an hour, a day,
a week. The idea is to catch things that only show up under real workload:
...
Now that you have completed the tutorial, explore further:

- The [Quickstart Guide](quickstart.md) covers additional scan options and
  configuration.
- The [Checks Reference](checks-reference.md) describes all 57 checks in
  detail.
- The [Architecture](architecture.md) document covers internal design, module
...
Check whether the PostgreSQL process inside the container is ready to accept
connections.
...
The extension requires a shared preload library entry and a container restart.
...
Or run the binary directly after building from source:
